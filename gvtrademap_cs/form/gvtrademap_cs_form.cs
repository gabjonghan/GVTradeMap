...
//		private const int			TOOLTIP_INITIAL			= 7;		// 1/60単位
		...
		private Point				m_mouse_move;		// 마우스の이동量
		...
		// 마우스フック
		private globalmouse_hook	m_mouse_hook;		// 마우스フック
		...
		private Thread				m_load_map_t;		// 지도読み込みスレッド
		private Thread				m_load_info_t;		// 정보読み込みスレッド
		private Thread				m_share_t;			// 항로공유スレッド
		private Thread				m_chat_log_t;		// チャット분석スレッド
...
#endif
		//
		private	Point				m_memo_icon_pos;	// 메모아이콘追加用좌표
		...
		{
			// 윈도우위치, サイズの復元
			// 内부でOnMove()が呼ばれるため, ちょっといやな感じに설정する
		...
		private void m_main_window_Paint(object sender, PaintEventArgs e)
		{
			// メイン윈도우の업데이트
			// ダイア로그그리기時のために用意しておく
			// 背景を그리기する関数が불명のため, 一도背景色で全体を塗られる
...
//			m_lib.Device.Clear(Color.FromArgb(255,152,176,190));
      ...
      try
      {
            ...
            // 도시詳細
            draw_progress("도시詳細...", _LoadInfosStatus.StatusMessage,
                            x, y, size_x,
      ...
      {
        Console.WriteLine("그리기중の例外をキャッチ(로딩중)\n" + ex.StackTrace);
      }
...
//			debug_transform_pos(12714, 3565);
		...
		{
			// InitializeComponent()内で呼ばれる事例が보고されたため, 
					...
					if(is_show_menu_strip){
						...
						if(!is_inside_mouse_cursor_main_window()){
							...
							if(++m_tooltip_interval >= TOOLTIP_INITIAL){
								...
								if(str != null){
									// なにかツールチップがあれば표시する
									// 마우스の그리기後のサイズ분ずらしたいが, 取得のしかたが분からない
									try{
...
*/
		...
		private void screen_shot()
		{
			...
			if((size.Width <= 0)||(size.Height <= 0)){
				MessageBox.Show("항로정보がないため, SSを作成できません. ",
...
//			string	Data	= tcp_text.Download("gvtrademap.daa.jp", str, Encoding.UTF8);
		...
		/*-------------------------------------------------------------------------
		 多重시작時にすでに시작している 교역MAP C# をアクティブにする
		 まだ윈도우を作成していないこと
		---------------------------------------------------------------------------*/
