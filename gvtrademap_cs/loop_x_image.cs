...
namespace gvtrademap_cs
{
    ...
    class TextureUnit : IDisposable
    {
        ...
        private Size m_texture_size;            // 텍스쳐サイズ
                                                // 
    ...
    {
        private const int TEXSIZE_STEP = 512;       // 분割サイズ
        private const int HEIGHT_MARGIN = 200;      // 上下のスクロールマージン
        private const float SCALE_MIN = 0.15f;       // 縮소限界
        private const float SCALE_MAX = 2;		// 拡대限界
        private const int EARTH_WIDTH = 16384;        // 측량 가로 최대
        private const int EARTH_HEIGHT = 8192;        // 측량 세로 최대
        ...
        // 그리기支援デリゲート
        // 그리기時のX방향ループに対応し, オフセットを解決する
        // このデリゲートが何회呼ばれるかはクライアントサイズと画상サイズによる
        ...
        public bool CreateImage(string file_name)
        {
            ...
            try
            {
                ...
                Bitmap bitmap = ResizeImage(new Bitmap(file_name), (int)EARTH_WIDTH/4, (int)EARTH_HEIGHT/4);
