...
namespace gvo_base
{
	...
	{
		// 専用の変換テーブルを사용するため, 분解能변경不可
		protected const int			COMPASS_DIV			= 360;				// コンパスの분解能
																			// 180の倍数であること
		protected const int			COMPASS_1ST_STEP	= 2;				// 最初の검색時の간격
		protected const int			COMPASS_2ND_RANGE	= 4*2;				// 詳細검색時の範囲(+-なのでこの値の倍の範囲を調べる)
		protected const int			COMPASS_DIV_90		= COMPASS_DIV/4;	// 90度분の분解能
		protected const int			COMPASS_DIV_45		= COMPASS_DIV/8;	// 45度분の분解能
		...
		// 분석결과
		private Point				m_point;			// 분석後の測量좌표
		private int					m_days;				// 분석後の日数
		...
		// コンパス분석詳細用
		protected int				m_1st_com_index;	// 黄色矢印위치インデックス
...
*/	
		...
		private bool				m_enable_point;		// 測量좌표분석유효
		private bool				m_enable_days;		// 항해일수분석유효
		private bool				m_enable_angle;		// コンパス분석유효
		...
		public gvo_capture_base()
		{
			...
			// 분석유효フラグ
			m_enable_point	= true;		// 測量좌표분석유효
			m_enable_days	= true;		// 항해일수분석유효
			m_enable_angle	= true;		// コンパス분석유효
		...
		private void analize_point()
		{
            m_point.X = 0;
            m_point.Y = 0;
            int state = 0; // 0:先頭余白 1:X좌표분석中 2:X,Y区切り文字 3:Y좌표분석中
            for (int i = 0; i < 10; ++i)
            {
                int num;
